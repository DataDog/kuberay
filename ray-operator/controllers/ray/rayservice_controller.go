...
func (r *RayServiceReconciler) abortUpgradeIfTimedOut(ctx context.Context, rayServiceInstance *rayv1.RayService, pendingCluster *rayv1.RayCluster) (bool, error) {
	...
	r.Recorder.Eventf(rayServiceInstance, nil, corev1.EventTypeWarning, string(utils.UpgradeTimeout), string(utils.DeleteAction),
...
func (r *RayServiceReconciler) reconcileStableServeService(ctx context.Context, rayServiceInstance *rayv1.RayService, activeCluster *rayv1.RayCluster) error {
	...
	if errors.IsNotFound(err) {
		...
		if err := r.Create(ctx, newSvc); err != nil {
			r.Recorder.Eventf(rayServiceInstance, nil, corev1.EventTypeWarning, string(utils.FailedToCreateService), string(utils.CreateAction), "Failed to create stable serve service %s/%s: %v", rayServiceInstance.Namespace, stableName, err)
			return err
		}
		r.Recorder.Eventf(rayServiceInstance, nil, corev1.EventTypeNormal, string(utils.CreatedService), string(utils.CreateAction), "Created stable serve service %s/%s", rayServiceInstance.Namespace, stableName)
		return nil
	}
	if err != nil {
		return err
	...
	if err := r.Update(ctx, existingSvc); err != nil {
		r.Recorder.Eventf(rayServiceInstance, nil, corev1.EventTypeWarning, string(utils.FailedToUpdateService), string(utils.UpdateAction), "Failed to update stable serve service %s/%s: %v", rayServiceInstance.Namespace, stableName, err)
		return err
	}
	r.Recorder.Eventf(rayServiceInstance, nil, corev1.EventTypeNormal, string(utils.UpdatedService), string(utils.UpdateAction), "Updated stable serve service %s/%s selector to cluster %s", rayServiceInstance.Namespace, stableName, activeCluster.Name)
	return nil
}
